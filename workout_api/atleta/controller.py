...
async def post(db_session: DatabaseDependency, atleta_in: AtletaIn = Body(...)):
    ...
    try:
        ...
        # Manipular a exceção de integridade de dados (CPF duplicado)
...
@router.get(
    ...
    response_model=Page[AtletaResponse],  # Use List ao invés de list
)
async def query(
    ...
    atletas: list[AtletaResponse] = (await db_session.execute(query)).scalars().all()

    if not atletas:  # Se não houver atletas, retorne uma lista vazia
        return []
...
async def query(
    ...
    # Lista personalizada de AtletaOut com campos necessários
    atletas_customizados = []
    for atleta in atletas:
        atleta_customizado = AtletaOut(
            id=atleta.id,
            nome=atleta.nome,
            ...
            centro_treinamento=CentroTreinamentoOut(
                id=atleta.centro_treinamento.id,
                nome=atleta.centro_treinamento.nome,
                endereco=atleta.centro_treinamento.endereco,
                proprietario=atleta.centro_treinamento.proprietario,
            ),
            categoria=CategoriaOut(id=atleta.categoria.id, nome=atleta.categoria.nome),
        )
        atletas_customizados.append(atleta_customizado)

    return paginate(atletas_customizados)
